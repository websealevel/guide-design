...
- alignement à gauche (*range left* ou *flush left*) : le plus populaire. Ca marche bien, il faut juste faire attention au brisure de ligne à droite. Jouer sur tous les paramètres (word spacing, letter-spacing) pour rendre le visuel meilleur (pas trop crenelé, accidenté)
- alignement à droite : plutot pour du texte décoratif, pas facile à lire, plutot pour le fun
- centré : bien pour poster, advertise. Pas adapté aux textes longs
- justifié : un texte mal justifié est un enfer à lire. **Ne jamais justifier du texte dans une petite colonne**. 
- *em* et *en* : ce sont des unités de mesure variables adaptées à la font utilisée. *en* mesure la longueur d'un *N* dans la font utilisé, *em* mesure la longueur d'un *M* dans la font utilisée.
- établir une hiérarchie
	- les **headlines** sont au sommet de l'arbre, ils doivent être lus, vu en premier. Pas forcément massif, **les headlines doivent être les premiers trucs qu'on lit**. Ca peut être jouer sur le weight, la position (les tiers par exemple) ou l'espace autour d'eux. Il faut que ça attire l'attention.
	- **items de liste**: **font partie de leur paragraphe**. De l'espace mais pas trop.
	- **images et legendes** : doivent être lisible mais ne pas interferer avec le propos
- ajustement des fins de lignes (*smooth line endings*), que les bas de colonnes s'alignent, qu'il y ait pas d'élément orphelins etc : a faire sur l'epreuve finale, a la toute fin, pas tout de suite. Sinon un changement peut tout casser. Reserver ce travail à la fin, quand le texte ne bougera plus trop.
- **numéros de téléphone, url, date, email sont sur une ligne** (pas de retour à la ligne en plein milieu !)
- *leading* : **ajouter 2pt ou 3pt a la taille du texte concerné**. Ex, 10pt text, 12pt leading. Marche bien. Quand texte est long, rajouter un peu de leading, mais pas trop, sinon on a tendance a revenir sur la même ligne quand on passe à la suivante.
- *paragraph spacing*: ajouter un *space-after* de 1.5mm entre 2 paragraphes (espace blanc) donne un effet sympa. Marche aussi avec un *space-before*
...
- *dashes* : 
	+ *hyphen*: `-` 
	+ *en dash* ou [tiret demi-cadratin](https://fr.wiktionary.org/wiki/demi-cadratin#fr): `–` , ou un range entre 2 nombres dans une adresse par exemple, 22–24. Ou pour espacer des points de conduite.
	+ *em dash* ou [tiret cadratin](https://fr.wiktionary.org/wiki/%E2%80%94) : a utiliser pour séparer 2 propositions dans une phrase – comme cela –
- ne jamais utiliser `&` sauf dans un nom propre d'entreprise
- utiliser les bonnes quotes ('virgules flottantes')
